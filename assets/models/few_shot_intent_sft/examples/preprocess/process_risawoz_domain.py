...
def get_args():
    ...
    parser.add_argument(
        "--output_file",
        default=(project_path / "data/ri_sawoz_domain.jsonl"),
        type=str
    )
...
label_map = DatasetLabels.label_map["ri_sawoz_domain"]
    ...
    with open(args.output_file, "w", encoding="utf-8") as fout:
        for split, dataset in dataset_dict.items():
            for sample in tqdm(dataset):
                ...
                for user_utterance, turn_domain in zip(dialogue["user_utterance"], dialogue["turn_domain"]):
